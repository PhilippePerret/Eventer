class EventsPanel extends Panel {
  ...
  uiMode() {
    if (this.editing) {
      const prop = this.editableProperties()[this.editingPropertyIndex] || 'text'
      const suffix = prop.charAt(0).toUpperCase() + prop.slice(1)
      const mode = `eventEditing${suffix}`
      return (APP_CONFIG.uiModes && APP_CONFIG.uiModes[mode]) ? mode : 'eventEditingText'
    }

    return this.filtered ? 'eventsFiltered' : 'events'
  }

  showAllEvents() {
    this.items = this.db.evenements
    this.filtered = false
  ...
  renderFilterClearButton() {
    ...
    button.textContent = 'Tous les events'
    button.addEventListener('click', () => this.showAllEvents())
    return button
  }

  render() {
    super.render()
    const button = this.renderFilterClearButton()
    if (button && this.container) this.container.prepend(button)
  }
