class Panel {
  ...
  uiMode() {
    if (this.editing && this.editingModeName) {
      const prop = this.editableProperties()[this.editingPropertyIndex] || ''
      const suffix = prop ? prop.charAt(0).toUpperCase() + prop.slice(1) : ''
      const propertyMode = `${this.editingModeName}${suffix}`

      if (APP_CONFIG.uiModes && APP_CONFIG.uiModes[propertyMode]) {
        return propertyMode
      }

      return this.editingModeName
    }
  ...
  render() {
    ...
    if (typeof updateShortcutsFooter === 'function') {
      updateShortcutsFooter()
    }
  }
